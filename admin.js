...
addProductBtn.addEventListener("click", async () => {
    ...
    const endpoint = editingProductId
        ? `${API_BASE}/admin/update-product/${editingProductId}`
...
async function loadProducts() {
    ...
    try {
        ...
        products.forEach((p) => {
            ...
            // Encode long filenames to prevent URL issues
            const safeImageUrl = p.image_url ? `${API_BASE}/uploads/${encodeURIComponent(p.image_url)}` : "";

            card.innerHTML = `
                <img src="${safeImageUrl || 'https://via.placeholder.com/250x250?text=No+Image'}" alt="${p.title}" />
                <h3>${p.title}</h3>
                <p>Price: $${p.price}</p>
                <p>Stock: ${p.stock}</p>
            ...
            card.querySelector(".delete-btn").addEventListener("click", async () => {
                ...
                try {
                    const res = await fetch(`${API_BASE}/admin/delete-product/${p.id}`, { method: "DELETE" });
                    const data = await res.json();
